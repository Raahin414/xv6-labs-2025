...
static int has_exec;              // whether -exec was provided
static char *execv_base[MAXARG];  // base argv for the command after -exec
static int execv_basec;           // count of base argv entries
...
main(int argc, char *argv[])
{
  ...
  if (argc >= 4 && strcmp(argv[3], "-exec") == 0) {
    ...
    // argv[4..argc-1] are the base command + its args
